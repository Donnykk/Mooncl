...
router.post("/login", async (req: Request, res: Response) => {
  ...
  if (!address) {
    return res
      .status(400)
      .json({ success: false, reason: "Address is required." });
  }
  // 生成一个随机挑战
  ...
  if (address instanceof Uint8Array) {
    ...
    console.log(
      `Player ${hexAddress} login initiated with challenge ${challenge}`
    );
  } else {
    console.log(
      `Player ${address} login initiated with challenge ${challenge}`
    );
  }
...
router.post("/login_signature", async (req: Request, res: Response) => {
  ...
  if (!challenge || !address) {
    return res
      .status(400)
      .json({ success: false, reason: "Address and challenge are required." });
  }
  // 验证挑战是否匹配
  if (loginChallenges[address] !== challenge) {
    return res
      ...
      .json({
        success: false,
        reason: "Challenge mismatch. Please initiate login again.",
      });
  }
  try {
    // 区分 Sui 和 sui 签名
    if (address instanceof Uint8Array) {
      ...
      if (
        (await verifysuiSignature(hexAddress, challenge, signature)) === false
      ) {
 ...
 */
router.get(
  "/replies/session/:target_address/:storyId",
  authenticate,
  async (req: Request, res: Response) => {
    const address = (req as any).userAddress;
    ...
    const storyId = Number(req.params.storyId);
    try {
      const replies = await ReplyService.getRepliesBetweenAddressesForStory(
        address,
        targetAddress,
        storyId
      );
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const stories = await Promise.all(
        receivedStories.map(
          async (storyId: string) => await getStoryById(storyId)
        )
      );
 ...
 */
router.get(
  "/stories/contract_address/:storyId",
  authenticate,
  async (req: Request, res: Response) => {
    try {
      const { storyId } = req.params;
      if (!storyId) {
        return res
          .status(400)
          .json({ success: false, reason: "storyId is required." });
      }
 ...
 */
router.get(
  "/stories/story_by_contract/:contractAddress",
  authenticate,
  async (req: Request, res: Response) => {
    try {
      ...
      if (!contractAddress) {
        return res
          .status(400)
          .json({ success: false, reason: "contractAddress is required." });
      }
      const result = await StoryService.getStoryByContractAddress(
        contractAddress
      );
      res.json({ success: true, result });
    } catch (error: any) {
    ...
    try {
      ...
      if (!storyId || !contractAddress) {
        return res
          ...
          .json({
            success: false,
            reason: "storyId and contractAddress are required.",
          });
      }
      const result = await StoryService.updateStoryContractAddress(
        storyId,
        contractAddress
      );
      res.json({ success: true, result });
...
router.post("/store_chat_history", async (req, res) => {
  ...
  if (!user_id || !role || !content) {
    return res
      .status(400)
      .json({ error: "user_id, role, and content are required" });
  }

  if (!["user", "ai"].includes(role)) {
    return res
      .status(400)
      .json({ error: "Invalid role. Must be 'user' or 'ai'." });
  }
...
router.put("/update_intimacy", async (req, res) => {
  ...
  if (!user_id || !new_intimacy) {
    return res
      .status(400)
      .json({ error: "user_id and new_intimacy are required" });
  }
  try {
...
router.post("/createTxn", async (req, res) => {
  ...
  const response = await TxnService.createNewTransaction(
    objectId,
    replyId,
    sender,
    receiver,
...
router.get("/getTxnByReply/:replyId", async (req, res) => {
  ...
  if (!replyId) {
    return res
      .status(400)
      .json({ success: false, message: "Reply ID is required" });
  }
...
router.put("/updateTxn", async (req, res) => {
  ...
  if (!objectId || !status) {
    return res
      .status(400)
      .json({ success: false, message: "Object ID and status are required" });
  }
...
router.post("/claim", async (req, res) => {
  ...
  if (!receiver || !objectId) {
    return res
      .status(400)
      .json({ success: false, message: "Receiver and Object ID are required" });
  }
...
router.post("/userSession", async (req, res) => {
  ...
  try {
    const response = await TxnService.startUserSession(
      txnObjectId,
      sessionId,
      userAddress,
      receiverAddress,
